    ...
    <div class="min-h-screen flex">
        ...
        <aside class="w-64 flex-shrink-0 bg-gradient-to-b from-cyan-900 to-cyan-950 text-white flex flex-col">
            <div class="p-6 border-b border-white/10 flex items-center gap-3">
                ...
                <div>
                    <div class="text-lg font-black tracking-tight leading-none">RelaXin</div>
                    <div class="text-[11px] text-cyan-300/70 font-semibold tracking-widest uppercase mt-0.5">Admin Panel</div>
                </div>
            </div>

            <nav class="mt-4 px-3 space-y-1 flex-1">
                <p class="px-3 pt-2 pb-1 text-[10px] font-bold uppercase tracking-widest text-cyan-400/50">Menu Utama</p>
                ...
                <p class="px-3 pt-4 pb-1 text-[10px] font-bold uppercase tracking-widest text-cyan-400/50">Operasional</p>

                <a href="{{ route('admin.reservasi') }}"
                   class="flex items-center justify-between gap-3 px-3 py-2.5 rounded-lg text-sm font-semibold transition
                    ...
                    @if(isset($pendingCount) && $pendingCount > 0)
                        <span class="text-[10px] font-bold bg-rose-500 text-white rounded-full px-1.5 py-0.5 min-w-[18px] text-center">{{ $pendingCount }}</span>
                    @endif
                </a>
            </nav>
            ...
            <div class="p-4 border-t border-white/10">
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="w-full flex items-center gap-2 justify-center text-xs font-semibold text-rose-200 hover:text-white bg-rose-500/10 hover:bg-rose-500/20 rounded-lg py-2 transition">
