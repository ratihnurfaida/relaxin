    ...
    <div style="font-family: 'Inter', sans-serif;">
        <div class="w-full bg-white border border-cyan-100 rounded-2xl p-8 shadow-xl shadow-cyan-600/10">
            ...
            <div class="text-center mb-6">
                <h1 class="text-2xl mb-0.5" style="font-family: 'Fraunces', serif; font-weight: 600; color: #155E75;">Selamat Datang</h1>
                <p class="text-gray-500 text-base">Masuk ke akun Relaxin kamu</p>
            </div>

            {{-- Alert Status Sesi --}}
            @if (session('status'))
                <div class="bg-cyan-50 text-cyan-800 text-base rounded-xl px-4 py-3 mb-4 border border-cyan-100">
                    {{ session('status') }}
                </div>
            @endif
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-4">
                ...
                <div>
                    <div class="flex items-center justify-between mb-1.5">
                        <label for="password" class="block text-base font-bold text-gray-700">Password</label>
                        @if (Route::has('password.request'))
                            <a href="{{ route('password.request') }}" class="text-sm font-bold hover:text-cyan-700 hover:underline" style="color: #0E7490;">Lupa password?</a>
                        @endif
                    </div>
                    <div class="relative">
                        <input
                ...
                <div class="flex items-center gap-2 pt-1">
                    ...
                    >
                    <label for="remember_me" class="text-base text-gray-500 font-medium cursor-pointer select-none">Ingat saya</label>
                </div>
            ...
            @if (Route::has('register'))
                <p class="text-center text-base text-gray-500 mt-6 font-medium">
                    Belum punya akun?
                    <a href="{{ route('register') }}" class="font-bold hover:text-cyan-700 hover:underline" style="color: #0E7490;">Daftar sekarang</a>
                </p>
            @endif
