...
<div class="pt-24 pb-16 px-4 md:px-8 max-w-3xl mx-auto">
    ...
    <div class="mb-8">
        ...
        <p class="text-slate-400 text-sm mt-1">Hotel: <span class="font-semibold text-slate-600">{{ $hotel->nama }}</span></p>
    </div>
    ...
    <div class="bg-white rounded-3xl shadow-cyan-lg p-8">
        <form method="POST" action="{{ route('admin.kamar.store') }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    <label for="kode_kamar" class="block text-sm font-semibold text-slate-700 mb-1.5">Kode Kamar <span class="text-rose">*</span></label>
                    <input
                        id="kode_kamar"
                        type="text"
                        name="kode_kamar"
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    >
                    @error('kapasitas') <p class="text-rose text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label for="total_kamar" class="block text-sm font-semibold text-slate-700 mb-1.5">Total Stok Kamar <span class="text-rose">*</span></label>
                    <input
                        id="total_kamar"
                        type="number"
                        name="total_kamar"
                    ...
                    >
                    @error('total_kamar') <p class="text-rose text-xs mt-1">{{ $message }}</p> @enderror
                </div>
            </div>

            {{-- Foto Kamar --}}
            <div>
                <label for="gambar" class="block text-sm font-semibold text-slate-700 mb-1.5">Foto Kamar</label>
                <input
                    id="gambar"
                    type="file"
                    name="gambar"
