...
<div class="pt-24 pb-16 px-4 md:px-8 max-w-3xl mx-auto">
    ...
    <div class="mb-8">
        ...
        <p class="text-slate-400 text-sm mt-1">
            Kamar <span class="font-semibold text-slate-600">{{ $room->room_number }}</span> –
            {{ $hotel->name }}
        </p>
    </div>

    <div class="bg-white rounded-3xl shadow-cyan-lg p-8">
        <form method="POST" action="{{ route('hotels.rooms.update', [$hotel, $room]) }}" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input
                        ...
                        class="w-full rounded-xl border border-slate-200 bg-ice-cyan/50 px-4 py-2.5 text-sm
                               focus:outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary
                               @error('room_number') border-rose @enderror"
                    >
            ...
            {{-- Foto Kamar --}}
            <div>
                <label for="photo" class="block text-sm font-semibold text-slate-700 mb-1.5">Ganti Foto Kamar</label>
                @if ($room->photo)
                    <div class="mb-2">
                        <img src="{{ asset('storage/' . $room->photo) }}" alt="Foto kamar saat ini" class="h-32 w-auto rounded-xl object-cover">
