<x-app-layout>
    ...
    <div class="min-h-screen bg-slate-50 flex flex-col lg:flex-row">
        ...
        {{-- ── KIRI: SIDE PANEL (Clean & Minimalist Split) ── --}}
        <aside class="w-full lg:w-5/12 bg-cyan-950 p-8 lg:p-12 flex flex-col justify-between lg:sticky lg:top-0 lg:h-screen">
            {{-- Bagian Atas: Logo / Nama Brand --}}
            <div>
                <span class="text-xs font-bold tracking-widest text-cyan-400 uppercase">RelaXin Hotels</span>
            </div>
    
            {{-- Bagian Tengah: Headline yang Clean --}}
            <div class="my-12 lg:my-0">
                <h1 class="text-4xl lg:text-5xl font-extrabold text-white tracking-tight leading-tight">
                    Your perfect<br>
                    <span class="text-cyan-400">escape</span> awaits
                </h1>
                <p class="text-cyan-200/60 text-sm mt-4 max-w-sm leading-relaxed">
                    Satu langkah lagi untuk menikmati pengalaman menginap terbaik dengan kenyamanan ekstra.
                </p>
            </div>
    
            {{-- Bagian Bawah: Ringkasan Hotel Pilihan --}}
            @isset($hotel)
            <div class="border-t border-white/10 pt-6">
                <p class="text-xs text-cyan-400/70 uppercase tracking-wider font-semibold mb-2">Destinasi Anda</p>
                <h3 class="text-xl font-bold text-white">{{ $hotel->nama }}</h3>
                <p class="text-sm text-cyan-200/70 mt-1">📍 {{ $hotel->kota ?? 'Bandung' }}</p>
            </div>
            @endisset
        </aside>
    
        {{-- ── KANAN: FORM PANEL (Minimalis & Teratur) ── --}}
        <main class="w-full lg:w-7/12 bg-white px-6 py-10 md:px-12 lg:py-14 overflow-y-auto">
            ...
            <form action="{{ route('booking.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                ...
                <div class="space-y-5">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="nama_tamu" value="{{ old('nama_tamu', auth()->user()->name ?? '') }}" placeholder="Nama sesuai identitas" class="w-full px-3 py-2 bg-slate-50/50 border border-slate-200 rounded-lg text-sm text-slate-800 focus:outline-none focus:border-cyan-600 focus:bg-white transition" required>
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-slate-600 mb-1">Email <span class="text-red-500">*</span></label>
                            <input type="email" name="email_tamu" value="{{ old('email_tamu', auth()->user()->email ?? '') }}" placeholder="alamat@email.com" class="w-full px-3 py-2 bg-slate-50/50 border border-slate-200 rounded-lg text-sm text-slate-800 focus:outline-none focus:border-cyan-600 focus:bg-white transition" required>
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-slate-600 mb-1">No. Telepon <span class="text-red-500">*</span></label>
                            <input type="tel" name="no_telepon" value="{{ old('no_telepon') }}" placeholder="Contoh: 0812345678" class="w-full px-3 py-2 bg-slate-50/50 border border-slate-200 rounded-lg text-sm text-slate-800 focus:outline-none focus:border-cyan-600 focus:bg-white transition" required>
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-slate-600 mb-1">No. KTP / Paspor</label>
                            <input type="text" name="no_identitas" value="{{ old('no_identitas') }}" placeholder="Nomor KTP" class="w-full px-3 py-2 bg-slate-50/50 border border-slate-200 rounded-lg text-sm text-slate-800 focus:outline-none focus:border-cyan-600 focus:bg-white transition">
                        </div>
                    </div>
                </div>
    
                {{-- SECTION 2: DETAIL PENGINAPAN --}}
                <div class="space-y-5 pt-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="date" name="tgl_checkin" id="tgl_checkin" value="{{ old('tgl_checkin') }}" min="{{ date('Y-m-d') }}" class="w-full px-3 py-2 bg-slate-50/50 border border-slate-200 rounded-lg text-sm text-slate-800 focus:outline-none focus:border-cyan-600 focus:bg-white transition" required>
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-slate-600 mb-1">Tanggal Check-out <span class="text-red-500">*</span></label>
                            <input type="date" name="tgl_checkout" id="tgl_checkout" value="{{ old('tgl_checkout') }}" min="{{ date('Y-m-d', strtotime('+1 day')) }}" class="w-full px-3 py-2 bg-slate-50/50 border border-slate-200 rounded-lg text-sm text-slate-800 focus:outline-none focus:border-cyan-600 focus:bg-white transition" required>
                        </div>
                        <div>
                            <label class="block text-xs font-medium text-slate-600 mb-1">Tipe Kamar <span class="text-red-500">*</span></label>
                            <select name="id_kamar" id="id_kamar" class="w-full px-3 py-2 bg-slate-50/50 border border-slate-200 rounded-lg text-sm text-slate-800 focus:outline-none focus:border-cyan-600 focus:bg-white transition cursor-pointer" required>
                        ...
                        <div>
                            ...
                            <select name="jumlah_kamar" id="jumlah_kamar" class="w-full px-3 py-2 bg-slate-50/50 border border-slate-200 rounded-lg text-sm text-slate-800 focus:outline-none focus:border-cyan-600 focus:bg-white transition cursor-pointer" required>
                                @for($i = 1; $i <= 5; $i++)
                        ...
                        <div>
                            ...
                            <input type="number" name="total_tamu" id="tamuInput" min="1" max="10" value="{{ old('total_tamu', 2) }}" class="w-full px-3 py-2 bg-slate-50/50 border border-slate-200 rounded-lg text-sm text-slate-800 focus:outline-none focus:border-cyan-600 focus:bg-white transition" required>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-3 pt-4">
                    ...
                    <textarea name="catatan" placeholder="Catatan tambahan (misal: bed tambahan, tipe single/twin bed, dll)..." rows="3" class="w-full px-3 py-2 bg-slate-50/50 border border-slate-200 rounded-lg text-sm text-slate-800 focus:outline-none focus:border-cyan-600 focus:bg-white transition resize-none">{{ old('catatan') }}</textarea>
                </div>
    
                {{-- SECTION 4: PEMBAYARAN --}}
                <div class="space-y-4 pt-4">
                    <h3 class="text-xs font-bold uppercase tracking-wider text-slate-400">Metode Pembayaran</h3>
                    <select name="metode_payment" id="metode_payment" class="w-full px-3 py-2 bg-slate-50/50 border border-slate-200 rounded-lg text-sm text-slate-800 focus:outline-none focus:border-cyan-600 focus:bg-white transition cursor-pointer" required>
                        <option value="" disabled {{ old('metode_payment') ? '' : 'selected' }}>Pilih metode pembayaran</option>
                        <option value="transfer_bank" {{ old('metode_payment') == 'transfer_bank' ? 'selected' : '' }}>Transfer Bank</option>
                        <option value="kartu_kredit" {{ old('metode_payment') == 'kartu_kredit' ? 'selected' : '' }}>Kartu Kredit / Debit</option>
                        <option value="bayar_di_hotel" {{ old('metode_payment') == 'bayar_di_hotel' ? 'selected' : '' }}>Bayar di Hotel</option>
                    </select>
    
                    {{-- Detail Rekening Bank Kontainer Tipis --}}
                    <div id="bankTransferDetails" class="hidden p-4 bg-slate-50 border border-slate-200 rounded-lg space-y-3">
                        <p class="text-xs text-slate-600 leading-relaxed">
                            Silakan lakukan transfer ke rekening resmi berikut:<br>
                            <span class="font-bold text-slate-800">Bank BCA — 12345678 — a.n RelaXin Hotel</span>
                        </p>
                        <div class="space-y-1">
                            <label class="block text-xs font-medium text-slate-700">Bukti Transfer <span class="text-cyan-600">*</span></label>
                            <input type="file" name="bukti_payment" id="bukti_payment" class="block w-full text-xs text-slate-500 file:mr-3 file:py-1.5 file:px-3 file:rounded-md file:border-0 file:text-xs file:font-semibold file:bg-cyan-950 file:text-white hover:file:bg-cyan-900 cursor-pointer">
                        </div>
                    </div>
                </div>
    
                {{-- ── RINGKASAN BIAYA SLIM ── --}}
